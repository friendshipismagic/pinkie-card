...
\item âge : 27 ans
\item profession : designer web
\item centres d'intérêt : technologie, réseaux sociaux, culture populaire
\item caractéristiques : bonne communication, ouverture d'esprit
\item habite en ville

\item Scénario associé :

Il a entendu parler du système par une publicité. Il s'attable et le serveur lui apporte une tablette exposant le système. Il commande et une fois servi le serveur reprend l'appareil et le reste du repas se déroule sans accroc.
...
\begin{itemize}
\item Persona 2 : Julien Deschamps

\begin{itemize}
\item âge : 20 ans
\item profession : étudiant
\item centres d'intérêts : musique, dessin
\item habitué aux fast-foods modernes et aux pizzas, kebab etc.
\item Scénario associé :

Julien se rend dans son restaurant préféré, le Kebab gourmet. Il utilise son téléphone pour commander depuis sa chambre. Sa commande est enregistrée en moins d'une minute. Il finit ses devoirs quand son téléphone émet une notification disant que sa commande sera prête dans cinq minutes. Il se met en route pour le restaurant, et rencontre Grégoire. Grégoire va aussi déguster un kebab. Ils entrent tous les deux dans le restaurant. Le kebab de Julien est prêt, Grégoire doit attendre un peu. Le paiement a été fait en ligne, ils peuvent sortir et déjeuner.
\end{itemize}
\end{itemize}
